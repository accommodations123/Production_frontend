...
export const PropertyCard = React.memo(({ property }) => {
    ...
    const propertyData = {
        ...
        socials: {
            whatsapp: property.Host?.whatsapp || property.host?.whatsapp || property.Host?.phone || property.host?.phone || property.phone || "",
            instagram: property.Host?.instagram || property.host?.instagram || property.Host?.User?.instagram || property.creator?.instagram || "",
            facebook: property.Host?.facebook || property.host?.facebook || property.Host?.User?.facebook || property.creator?.facebook || "",
            twitter: property.Host?.twitter || property.host?.twitter || property.Host?.x || property.Host?.User?.twitter || "",
            email: property.Host?.email || property.host?.email || property.Host?.User?.email || property.host?.User?.email || property.creator?.email || property.email || ""
        }
    };
    ...
    return (
        <CardContainer key={propertyData.id} linkTo={`/rooms/${propertyData.id}`}>
            ...
            {/* Content Section */}
            <div className="p-4 sm:p-5 flex-grow flex flex-col gap-3.5">
                {/* Title & Location */}
                ...
                {/* Price & Actions Row */}
                <div className="flex items-end justify-between mt-auto pt-3.5 border-t border-border/60">
                    <div className="flex flex-col">
                        <div className="flex items-baseline gap-1">
                            ...
                            {propertyData.price.amount > 0 && (
                                <span className="text-xs font-medium text-muted-foreground">/{propertyData.price.period}</span>
                            )}
                        </div>
                    </div>
