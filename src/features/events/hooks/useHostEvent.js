...
import { hostEventService, compressImage } from "@/features/events/services/hostEventService"
...
export const useHostEvent = () => {
    ...
    const createOrUpdateEvent = async () => {
        ...
        try {
            ...
            // Split on newlines and/or commas since the UI is a textarea (one item per line)
            if (formData.what_is_included) {
                venuePayload.included_items = typeof formData.what_is_included === 'string'
                    ? formData.what_is_included.split(/[\n,]+/).map(s => s.trim()).filter(Boolean)
